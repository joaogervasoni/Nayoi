const { formatEmojiId } = require("../../utils/functions.js");
...
module.exports.run = async (bot, message, args, lang) => {
    try {
        ...
        if (!subcmd) return message.reply(lang.returnEspaceError);

        let emoji = bot.emojis.cache.get(formatEmojiId(cmd));
        let emoji2 = bot.emojis.cache.get(formatEmojiId(subcmd));

        if (!emoji || !emoji2) return message.reply(lang.returnInvalid);
